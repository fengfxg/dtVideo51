...
DECLARE_LOG_CATEGORY_EXTERN(DTMP4, Log, VeryVerbose);

DEFINE_LOG_CATEGORY(DTMP4);
DEFINE_LOG_CATEGORY(DTWMF);

WINDOWSPLATFORMFEATURES_START

bool FDTWmfMp4Writer::Initialize(const TCHAR* Filename)
{
	CHECK_HR(MFCreateSinkWriterFromURL(Filename, nullptr, nullptr, Writer.GetInitReference()));
	UE_LOG(DTWMF, Verbose, TEXT("Initialised Mp4Writer for %s"), Filename);
	return true;
}

TOptional<DWORD> FDTWmfMp4Writer::CreateAudioStream(const FString& Codec, const AVEncoder::FAudioConfig& Config)
{
	GUID Format;
	...
	{
		UE_LOG(DTWMF, Error, TEXT("FDTWmfMp4Writer doesn't support codec %s"), *Codec);
		return {};
	}
...
TOptional<DWORD> FDTWmfMp4Writer::CreateVideoStream(const FString& Codec, const AVEncoder::FVideoConfig& Config)
{
	GUID Format;
	if (Codec == "h264")
	{
...
bool FDTWmfMp4Writer::Start()
{
	CHECK_HR(Writer->BeginWriting());
	return true;
}

bool FDTWmfMp4Writer::Write(const AVEncoder::FMediaPacket& InSample, DWORD StreamIndex)
{
	...
	UE_LOG(DTMP4, VeryVerbose, TEXT("stream #%d: time %.3f, duration %.3f%s"), StreamIndex, Sample.GetTime().GetTotalSeconds(), Sample.GetDuration().GetTotalSeconds(), InSample.IsVideoKeyFrame() ? TEXT(", key-frame") : TEXT(""));

	return true;
}

bool FDTWmfMp4Writer::Finalize()
{
	CHECK_HR(Writer->Finalize());
	UE_LOG(DTWMF, VeryVerbose, TEXT("Closed .mp4"));
